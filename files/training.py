...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # get test epoch metrics
            val_res = self.test_epoch(dl_val)
            val_acc.append(val_res.accuracy)
            val_loss.append(sum(val_res.losses) / len(val_res.losses))
            ...
            if post_epoch_fn:
                post_epoch_fn(epoch, train_res, val_res, verbose)
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss = self.loss_fn(outputs.float(), y)
        loss.backward()
        loss = loss.item()
        self.optimizer.step()

        # calculate num correct
        max_preds = torch.max(outputs, 1)
        compare = torch.eq(max_preds.indices, y)
        num_correct = torch.sum(compare).item()
        # ========================
