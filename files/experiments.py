...
from utils.models import ConvClassifier
...
def run_experiment(run_name, out_dir='./results', seed=None,
    ...
    #ds_train1 = CIFAR10(root=DATA_DIR, download=True, train=True, transform=tf)
    #ds_train = torch.cat((ds_train,ds_train1),1)
    ...
    #ds_test2 = CIFAR10(root=DATA_DIR, download=True, train=False, transform=tf)
    #ds_test = torch.cat((ds_test,ds_test2),1)
    ...
    # - Create optimizer - torch.optim.Adam/torch.optim.SGD, with the given lr and reg as weight_decay.
    ...
    # create model, set optimizer, loss func, and trainer
    model = model_cls((3, 32, 32), 10, filters_per_layer * layers_per_block, pool_every, hidden_dims)
